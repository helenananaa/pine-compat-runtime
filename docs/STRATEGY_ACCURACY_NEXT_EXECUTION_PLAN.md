...
状态：阶段 A 已关闭；阶段 B–D 待执行。本文只定义开发顺序和验收门槛，不新增兼容性声明。
...
- [ ] 新增 `docs/STRATEGY_SESSION_RISK_BEHAVIOR_AUDIT.md`。
- [ ] 列出现有 `trading_day_key`、风险窗口键及其调用者，不直接替换所有 UTC 运算。
- [ ] 分别锁定 intraday loss、filled orders、consecutive loss days 的归属与重置规则。
- [ ] 为隔夜时段、多时段交易、非交易日、日线和高于日线周期定义目标子集。

门槛：明确区分自然日、交易日、交易时段和图表 bar；不假定所有风险规则用同一个键。

### B2：先定义宿主契约

- [ ] 评估宿主提供逐 bar 交易日/窗口标识，或提供已规范化的时段区间。
- [ ] 写清 schema 版本、时间单位、时区责任、区间端点、覆盖范围与多时段归属。
- [ ] 为重复、重叠、无序、缺失、未知版本和 live 更新冲突定义稳定诊断。
- [ ] 决定缺失输入是保留已文档化的 UTC 子集还是拒绝特定模式；不得默默声称 session 精确支持。
- [ ] 保证批量预检与增量未来输入规则一致，不要求流式宿主提供无限未来日历。

门槛：契约通过评审，所有输入可由本地 fixture 提供；核心不查询网络或维护交易所日历服务。

### B3：内部实现与模式一致性

- [ ] 先实现输入验证和只读窗口查询，再接入风险状态转换。
- [ ] 测试一个时段跨 UTC 午夜不错误重置、实际窗口切换只重置一次。
- [ ] 验证休市间隔、夏令时切换、分段时段、小于/等于/大于日线周期。
- [ ] 验证增量追加、重复 forming 更新、确认 bar 和 rollback 的窗口状态。
- [ ] 保留无新输入的旧子集回归，并明确允许的行为变化。

### B4：宿主与验收

- [ ] CLI、Python、WASM 使用同一契约解析规则和正负向 goldens。
- [ ] 涉及 RealtimeSession 时先审查 ABI 版本策略，不默认沿用也不无理由升级。
- [ ] 同步契约、诊断、风险文档和 conformance，运行定向测试及 `scripts/verify.sh`。
- [ ] audit 只声明被覆盖的 session 子集，不宣布完整交易所日历兼容。
...
- [ ] 收集原创、明确授权或许可允许使用的 v5/v6 策略；私有脚本不提交公开仓库。
- [ ] 记录来源、授权、版本、哈希、输入参数、symbol/timeframe 和数据覆盖。
- [ ] 固定语料清单并去重；缺数据样本与运行失败样本分开计数。
- [ ] 记录 parse、sema、runtime、输出可比对性和结果一致性五个独立指标。

### D2：按根因选择下一小片

- [ ] 将失败分为语言/类型、内置函数、request 数据契约、策略生命周期、账户精度和报告输出。
- [ ] 给每个根因记录受影响脚本数、严重程度、依赖、证据可获得性和估计复杂度。
- [ ] 优先修正已接受却执行错误的行为，再考虑新增接受范围。
- [ ] 数量默认值、精度、货币换算或报告字段只有被语料证明有价值时才立独立计划。
- [ ] 每次只选一个最小根因，抽取原创最小复现，执行与 A 相同的完整验收闭环。
...
- [ ] 源码、测试、fixture、goldens、矩阵和文档在同一行为切片内一致。
- [ ] 每个已接受的新行为都有正常、边界、负向和必要的模式交互证据。
- [ ] `scripts/verify.sh` 对最终代码成功执行，保留命令结果；不引用旧阶段通过数替代本次验证。
- [ ] 本地开发完成与远端 CI/合并/发布完成分别报告。
- [ ] 用户授权集成后，重新核验远端、分支差异和保护规则，再采用小切片 PR。
- [ ] 暂存只使用明确文件白名单；不把已有未跟踪文件或无关改动混入。
