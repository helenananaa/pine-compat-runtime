...
`currency=currency.NONE` and same-symbol `currency=currency.USD` are accepted
as the explicit form of that default no-conversion account-currency path;
...
use, and mutation. Explicit `strategy(..., currency=currency.NONE)` and
`strategy(..., currency=currency.USD)` select the same no-conversion path;
other currency values, settings overrides, and cross-currency conversion remain
unsupported. The same-currency subset adds
...
input.int            supported
ta.sma               supported
ta.wma               supported    numeric-compatible length truncated toward zero like Pine int(); non-numeric length rejected; sma/hma/vwma length stay integer-compatible
ta.ema               supported
