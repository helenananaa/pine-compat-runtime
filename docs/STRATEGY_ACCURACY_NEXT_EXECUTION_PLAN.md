...
状态：阶段 A、C 已关闭；阶段 B 已完成 session 复核修复及重新验收；阶段 D1 仅完成初步盘点，冻结清单和测量证据未完成，D2 不实施新行为切片。本文不新增兼容性声明。
...
- [ ] 记录来源、授权、版本、哈希、输入参数、symbol/timeframe 和数据覆盖。当前只有汇总描述，尚缺逐样本 manifest。
- [ ] 固定语料清单并去重；缺数据样本与运行失败样本分开计数。尚缺冻结清单和去重报告。
- [ ] 记录 parse、sema、runtime、输出可比对性和结果一致性五个独立指标。文件/快照数量不等于分阶段实测通过数。

### D2：按根因选择下一小片

- [x] 将失败分为语言/类型、内置函数、request 数据契约、策略生命周期、账户精度和报告输出。
- [ ] 给每个根因记录受影响脚本数、严重程度、依赖、证据可获得性和估计复杂度。当前只有粗分类，尚无逐根因排名证据。
